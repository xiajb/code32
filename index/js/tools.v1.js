	...
	login: {
		...
		windows: function(){
			var d = dialog({
					id: "windowLogin",
					title: "您尚未登陆",
					content: '<iframe id="test-iframe" width="100%" height="99%" frameborder="0" framespacing="0" src="http://www.qfdlqz.com/windows/login_register.aspx?callback=' + "norefresh" + '" scrolling="no"></iframe>',
					align: 'left',
					fixed: true,
					zIndex: 1050,
					padding: 0,
		 ...
		 */
		page: function(){
			window.top.location.href = "http://www.qfdlqz.com/newlogin.aspx";
		}
			...
			var
				$f = $("<form>")
						.attr({
							"id": "cart-oneform",
							"method": "post",
							"action": "http://www.qfdlqz.com/pay/cart.aspx"
						})
						,
	...
	order: {
		...
		create: function (buyJSON) {
                    ...
                    */
                    ...
                    // 提交地址  
                    var action = "http://www.qfdlqz.com/pay/order-product.aspx";
                    // 创建Form  
			...
			var 
				$f = $("<form>")
						.attr({
							"id":"order-oneform",
							"method":"post",
							"action":"http://www.qfdlqz.com/pay/order-product.aspx"
						})
						,
				$hdn1 = $("<input type='hidden'>")
							.attr({
		...
		mesPanel: function(){
			...
			$(".message-expand").on("click","#btn-message",function(){
				...
				if(!(/^(13[0-9]|15[0-9]|17[0-9]|18[0-9])\d{8}$/).test(txtMobile)){
					...
					$.post("http://www.qfdlqz.com/ashx/studentsound/CreateMsg.ashx",$("#mesform").serialize())
					 .done(function(d){
							if (d == "留言内容过长") {
								var d = dialog({
									title: '温馨提示',
								...
								var d = dialog({
									title: '温馨提示',
									content: '留言成功!如紧急,请拨免费咨询:110-110-110',
									quickClose: true,
									align: 'top left',
									fixed: true,
									zIndex: 200000,
