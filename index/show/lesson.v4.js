...
$(function () {
        ...
        */
        CheckLogin: function (away) {
            ...
            if (login == null) {
                if (away == "0" || away == "") {
                    ...
                    $.fancybox.open({
                        href: 'http://www.qfdlqz.com/login/logins.html?callback= ' + escape(document.URL) + '',
                        type: 'iframe',
                        padding: 10,
                        width: '400',
                        height: '440',
        ...
        _download: function (lessname, _lessonid) {
            if (courses.CheckLogin(1)) {
                $.post("http://www.qfdlqz.com/ashx/loginstate.aspx").done(function (d) {
                    d = $.parseJSON(d);
    ...
    if ($lesson_20140110.length) {
        ...
        $("#add_cart-1").click(function (event) {
            //if (courses.CheckLogin(1)) {
            document.getElementById("course_form").action = "http://www.qfdlqz.com/pay/cart.aspx";
        ...
        $("#quick_buy").click(function (event) {
            if (courses.CheckLogin(1)) {
                document.getElementById("course_form").action = "http://www.qfdlqz.com/pay/order_course.aspx";
                document.getElementById("course_form").submit();
            }
        });
        ...
        function cislogin(away) {
            ...
            if (login == null) {
                if (away == "0" || away == "") {
                    window.parent.location.href = "http://www.qfdlqz.com/newlogin.aspx?callback=" + escape(document.URL);
                } else if (away == "1") {
                    $.fancybox.open({
                        href: 'http://www.qfdlqz.com/login/logins.html?callback= ' + escape(document.URL) + '',
                        type: 'iframe',
...
//                var d = dialog({
//                    title: "登陆",
//                    content: '<iframe id="test-iframe" width="100%" height="99%" frameborder="0" framespacing="0" src="http://www.qfdlqz.com/windows/login_register.aspx?callback=' + escape(document.URL) + '" scrolling="no"></iframe>',
//                    align: 'left',
//                    fixed: false,
//                    zIndex: 87,
//                    padding: 0,
...
//                });
            ...
            if (comment_val.replace("有什么感想，您也来说说吧！", "").length < 1) {
                ...
                if (cislogin(1)) {
                    ...
                    $.post("/ashx/lesson/postcousecomment.aspx", $("#comment_form").serialize()).done(function (d) {
                        ...
                        $.Boxen.open(window, {
                            url: "http://my.qfdlqz.com/emailregister/learnerRecommend/user_recommend5.aspx",
    ...
    if ($lesson_play_20140110.length) {
        ...
        $("#addCart").click(function () {
            ...
            //if (courses.CheckLogin(1)) {
            //                document.getElementById("lesson_form").action = "http://www.qfdlqz.com/pay/cart.aspx";
    ...
    if ($order_20140110.length) {
        ...
        $("#zhifubao").click(function () {
            ...
            $.Boxen.open(window, { url: "http://www.qfdlqz.com/pay/payTips1.html", urlParams: { "a": new Date().getMilliseconds(), "ordernumber": $("#ordernumber").val() }, width: 335, height: 210, title: "在线支付提示" });
        ...
        $("#wangyin").click(function () {
            $("#paytype").val('2');
            $.Boxen.open(window, { url: "http://www.qfdlqz.com/pay/wangyin.html",
